...
namespace MobUni.ApplicationCore.Validation
{
    public class ActivityValidator:AbstractValidator<CreateActivityDTO>
    {
        ...
        {
            RuleFor(a => a.ActivityStartTime).GreaterThan( DateTime.SpecifyKind(DateTime.Now, DateTimeKind.Utc)).LessThan(a=>a.ActivityEndTime).WithMessage("Başlangıç tarihi, bitiş tarihinden sonra veya geçmiş bir tarih olmamalıdır.");
            RuleFor(a => a.ActivityEndTime).GreaterThan( DateTime.SpecifyKind(DateTime.Now, DateTimeKind.Utc)).WithMessage("Bitiş tarihi geçmiş bir tarih içermemelidir.");
