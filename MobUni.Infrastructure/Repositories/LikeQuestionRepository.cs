...
namespace MobUni.Infrastructure.Repositories
{
    public class LikeQuestionRepository : EfRepositoryBase<LikeQuestion>, ILikeQuestionRepository
    {
        ...
        {
            return _mobUniDbContext.LikeQuestion.Where(question => question.IsActive).OrderByDescending(t => t.CreatedTime).ToList();
        }

        public List<LikeQuestion> GetLikedByUserId(string userId)
        {
            return _mobUniDbContext.LikeQuestion.Where(question => question.UserId == userId && question.IsActive).Include(likeQuestion=>likeQuestion.User).OrderByDescending(t => t.CreatedTime).ToList();
        }
